...
def load_data(dataset_name,musae_dataset_path=None,train_ratio=0.4,val_ratio=0.3):
    if dataset_name == 'Cora':
        ...
        path = os.path.join(os.getcwd(), 'data','Amazon_Photo')
        dataset = Amazon(path,name='Photo')
        data = dataset[0]
    elif dataset_name == 'Computer':
        path = os.path.join(os.getcwd(), 'data','Amazon_Computers')
        dataset = Amazon(path,name='Computers')
        data = dataset[0]
    elif dataset_name == "CS":
        path = os.path.join(os.getcwd(), 'data','Coauthor')
        ...
        print("here in OBGN-products")
        path = os.path.join(os.getcwd(), 'data','OGBN-Products')
        from ogb.nodeproppred import PygNodePropPredDataset, Evaluator
        dataset = PygNodePropPredDataset(name=dataset_name.lower(), transform=transforms.ToSparseTensor(), root=path)
        evaluator = Evaluator(name=dataset_name.lower())
        data = dataset.data
    ...
    elif dataset_name == "flipkart_10_10":
        data = pickle.load(open(src_dir+"/../data/flipkart/data_10_10.pkl","rb"))
    elif dataset_name == "flipkart_50":
        data = pickle.load(open(src_dir+"/../data/flipkart/data_50.pkl","rb"))
    else:
        import sys
    ...
    try:
        ...
        print("Train mask doesn't exist")
        list_all_ids = list(range(0, data.x.shape[0]))
