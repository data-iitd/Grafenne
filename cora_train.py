...
### This doesn't work thus commented but kept here for reference
# print(feature_mask.sum())
# tp = torch.zeros_like(feature_mask).bool()
# feature_mask = torch.where(data['x'].bool().to(device), feature_mask, tp)
# print("after removing edges where feature value is 1", feature_mask.sum(),data.x.sum() )
######
...
for epoch in range(0,num_epochs):
    ...
    total_computed = 0
    #feature_mask_tp = feature_mask.clone().detach().to(device)
    #feature_mask_tp
